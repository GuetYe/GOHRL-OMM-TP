...
def get_all_pairs_dijkstra_path_length():
    ...
    all_hops = nx.shortest_path(my_graph, source=3, target=14)
...
if __name__ == '__main__':
    ...
    LINKS_INFO = WORK_DIR / "DataSet/14/links_info.xml"  # 链路信息的xml文件路径
    PICKLE_PATH = WORK_DIR / "DataSet/14"  # 数据路径 weight_pickle
    ...
    with open("./picture/resault.json", "w") as f:
        json.dump(resault, f)
    ...
    with open("./picture/bw.json", "w") as f:
        json.dump(bw_data, f, indent=4)

    with open("./picture/delay.png.json", "w") as f:
        json.dump(delay_data, f, indent=4)

    with open("./picture/loss.json", "w") as f:
